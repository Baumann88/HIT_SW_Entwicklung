...
#define SDOControlWordEnable                       0x00000001
#define SDOControlWordWrite                        0x00000002
#define SDOCOntrolWordSend4Byte                    0x00000010
#define SDOControlBusNumMuster                     0x0000000c
#define SDOControlSendIDMuster                     0x000000e0
...
//SDO Send Word
static uint32 *SDOControlWord;
static uint32 *SDOContent;
...
void CANopenCommInit(void){
    ...
    SDOControlWord=BindODMem(DevParaStartIndex,SubindexSysSDOControlWord,TypeU32,0); *SDOControlWord=0x00000000;
    SDOContent=BindODMem(DevParaStartIndex,SubindexSysSDOContent,TypeU32,0); *SDOContent=0x00000000;
...
void CANBusChk(void){
    ...
    x_can_bus_get_status (X_CAN_BUS_03,&t_Status);
    if(t_Status.u16_BusState!=X_CAN_BUS_FAILURE_BUSOFF) *CANBusStatus&=~0x04;
    else *CANBusStatus|=0x04;

    x_can_bus_get_status (X_CAN_BUS_04,&t_Status);
    if(t_Status.u16_BusState!=X_CAN_BUS_FAILURE_BUSOFF) *CANBusStatus&=~0x08;
    else *CANBusStatus|=0x08;
}
...
void SDOHandle(void){
    ...
    //sdo send service
    if(*SDOControlWord&SDOControlWordEnable){
        if(*SDOControlWord&SDOControlWordWrite) {
            if(*SDOControlWord&SDOCOntrolWordSend4Byte) Data[0]=0x23;
            else Data[0]=0x2b;
        }
        else Data[0]=0x40;
        sendbus=(uint16)((*SDOControlWord&SDOControlBusNumMuster)>>2);
        sendId=(uint16)((*SDOControlWord&SDOControlSendIDMuster)>>5);
        Data[1]=(uint8)((*SDOControlWord&0x0000ff00)>>8);
        Data[2]=(uint8)((*SDOControlWord&0x00ff0000)>>16);
        Data[3]=(uint8)((*SDOControlWord&0xff000000)>>24);
        Data[4]=(uint8)(*SDOContent&0x000000ff);
        Data[5]=(uint8)((*SDOContent&0x0000ff00)>>8);
        Data[6]=(uint8)((*SDOContent&0x00ff0000)>>16);
        Data[7]=(uint8)((*SDOContent&0xff000000)>>24);

        col2_pdo_write(mpv_Bus[sendbus],RxSDO_COB+sendId,&Data[0],8);
        *SDOControlWord&=~SDOControlWordEnable;
    }
}
...
void NMTInit(void){
    ...
    GetSetOD(InverterStartIndex+0x0001,SubindexInverterNodeStatus,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[1].NMTStatus=(uint8 *)&ODMem[ODMemAddr];
    NodeNMTStatus[1].NodeStatus=(uint8 *)&ODMem[ODMemAddr]+1;
    GetSetOD(InverterStartIndex+0x0001,SubindexInverterNMT,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[1].NMTSend=(uint16 *)&ODMem[ODMemAddr];

    GetSetOD(InverterStartIndex+0x0002,SubindexInverterNodeStatus,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[2].NMTStatus=(uint8 *)&ODMem[ODMemAddr];
    NodeNMTStatus[2].NodeStatus=(uint8 *)&ODMem[ODMemAddr]+1;
    GetSetOD(InverterStartIndex+0x0002,SubindexInverterNMT,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[2].NMTSend=(uint16 *)&ODMem[ODMemAddr];

    GetSetOD(InverterStartIndex+0x0003,SubindexInverterNodeStatus,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[3].NMTStatus=(uint8 *)&ODMem[ODMemAddr];
    NodeNMTStatus[3].NodeStatus=(uint8 *)&ODMem[ODMemAddr]+1;
    GetSetOD(InverterStartIndex+0x0003,SubindexInverterNMT,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[3].NMTSend=(uint16 *)&ODMem[ODMemAddr];

    GetSetOD(InverterStartIndex+0x0004,SubindexInverterNodeStatus,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[4].NMTStatus=(uint8 *)&ODMem[ODMemAddr];
    NodeNMTStatus[4].NodeStatus=(uint8 *)&ODMem[ODMemAddr]+1;
    GetSetOD(InverterStartIndex+0x0004,SubindexInverterNMT,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[4].NMTSend=(uint16 *)&ODMem[ODMemAddr];

    GetSetOD(InverterStartIndex+0x0005,SubindexInverterNodeStatus,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[5].NMTStatus=(uint8 *)&ODMem[ODMemAddr];
    NodeNMTStatus[5].NodeStatus=(uint8 *)&ODMem[ODMemAddr]+1;
    GetSetOD(InverterStartIndex+0x0005,SubindexInverterNMT,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[5].NMTSend=(uint16 *)&ODMem[ODMemAddr];

    GetSetOD(InverterStartIndex+0x0006,SubindexInverterNodeStatus,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[6].NMTStatus=(uint8 *)&ODMem[ODMemAddr];
    NodeNMTStatus[6].NodeStatus=(uint8 *)&ODMem[ODMemAddr]+1;
    GetSetOD(InverterStartIndex+0x0006,SubindexInverterNMT,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[6].NMTSend=(uint16 *)&ODMem[ODMemAddr];

    GetSetOD(InverterStartIndex+0x0007,SubindexInverterNodeStatus,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[7].NMTStatus=(uint8 *)&ODMem[ODMemAddr];
    NodeNMTStatus[7].NodeStatus=(uint8 *)&ODMem[ODMemAddr]+1;
    GetSetOD(InverterStartIndex+0x0007,SubindexInverterNMT,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[7].NMTSend=(uint16 *)&ODMem[ODMemAddr];

    GetSetOD(RemoteControlStartIndex,SubindexRemoteCtrlNodeStatus,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[8].NMTStatus=(uint8 *)&ODMem[ODMemAddr];
    NodeNMTStatus[8].NodeStatus=(uint8 *)&ODMem[ODMemAddr]+1;
    GetSetOD(RemoteControlStartIndex,SubindexRemoteCtrlNMT,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[8].NMTSend=(uint16 *)&ODMem[ODMemAddr];

    GetSetOD(AutoDriveStartIndex,SubindexCamera1NodeStatus,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[9].NMTStatus=(uint8 *)&ODMem[ODMemAddr];
    NodeNMTStatus[9].NodeStatus=(uint8 *)&ODMem[ODMemAddr]+1;
    GetSetOD(AutoDriveStartIndex,SubindexCamera1NMT,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[9].NMTSend=(uint16 *)&ODMem[ODMemAddr];

    GetSetOD(AutoDriveStartIndex,SubindexCamera2NodeStatus,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[10].NMTStatus=(uint8 *)&ODMem[ODMemAddr];
    NodeNMTStatus[10].NodeStatus=(uint8 *)&ODMem[ODMemAddr]+1;
    GetSetOD(AutoDriveStartIndex,SubindexCamera2NMT,&ODMemAddr,&value,GetOD);
    NodeNMTStatus[10].NMTSend=(uint16 *)&ODMem[ODMemAddr];

    NodeNMTStatus[0].NodeID=IDInverter111;
    NodeNMTStatus[1].NodeID=IDInverter112;
    NodeNMTStatus[2].NodeID=IDInverter121;
    NodeNMTStatus[3].NodeID=IDInverter122;
    NodeNMTStatus[4].NodeID=IDInverter211;
    NodeNMTStatus[5].NodeID=IDInverter212;
    NodeNMTStatus[6].NodeID=IDInverter221;
    NodeNMTStatus[7].NodeID=IDInverter222;
    NodeNMTStatus[8].NodeID=IDRemoteControl;
    NodeNMTStatus[9].NodeID=IDCamera1;
    NodeNMTStatus[10].NodeID=IDCamera2;

    NodeNMTStatus[0].BusChannel=BusChInverter111;
    NodeNMTStatus[1].BusChannel=BusChInverter112;
    NodeNMTStatus[2].BusChannel=BusChInverter121;
    NodeNMTStatus[3].BusChannel=BusChInverter122;
    NodeNMTStatus[4].BusChannel=BusChInverter211;
    NodeNMTStatus[5].BusChannel=BusChInverter212;
    NodeNMTStatus[6].BusChannel=BusChInverter221;
    NodeNMTStatus[7].BusChannel=BusChInverter222;
    NodeNMTStatus[8].BusChannel=BusChRemoteControl;
    NodeNMTStatus[9].BusChannel=BusChCamera1;
    NodeNMTStatus[10].BusChannel=BusChCamera2;
