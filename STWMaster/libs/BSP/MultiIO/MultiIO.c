...
static uint32 *IOPortControlWord;
...
//=================================================================================================
/*!
    Function: Initial IO Port of Extern Board 02
    output: None
*/
//=================================================================================================
void MultiIOEB02Init(void){
    uint16 iPort;
    //Input Initial
    iPort=15;
    while(iPort<27){
        if(inputConfig[iPort][1]!=InputCfgNoCfg){
            x02_in_init(X_EXPBOARD_SLOT_A,inputConfig[iPort][0],X02_IN_TYPE_DIGITAL,0,(X02_IN_CIRCUIT_ACTIVE_HIGH|X02_IN_CIRCUIT_PULL_DOWN),0,NULL);
            *IOPortInputArray[iPort]=(uint32)InputCfgDigital<<16;
        }
        else
            *IOPortInputArray[iPort]=(uint32)InputCfgNoCfg<<16;
    ...
    while(iPort<39){
        if(inputConfig[iPort][1]!=InputCfgNoCfg){
            x02_in_init(X_EXPBOARD_SLOT_B,inputConfig[iPort][0],X02_IN_TYPE_DIGITAL,0,(X02_IN_CIRCUIT_ACTIVE_HIGH|X02_IN_CIRCUIT_PULL_DOWN),0,NULL);
            *IOPortInputArray[iPort]=(uint32)InputCfgDigital<<16;
        }
        else
            *IOPortInputArray[iPort]=(uint32)InputCfgNoCfg<<16;
    ...
    if(outputConfig[iPort][1]!=OutputCfgNoCfg){
        x02_out_init(X_EXPBOARD_SLOT_A,outputConfig[iPort][0], X_OUT_TYPE_DIGITAL, 0u,0u,NULL);
        *IOPortOutputArray[iPort]=(uint32)OutputCfgDigital<<16;
    }
    else
        *IOPortOutputArray[iPort]=(uint32)OutputCfgNoCfg<<16;
    ...
    iPort=9;
    if(outputConfig[iPort][1]!=OutputCfgNoCfg){
        x02_out_init(X_EXPBOARD_SLOT_B,outputConfig[iPort][0], X_OUT_TYPE_DIGITAL, 0u,0u,NULL);
        *IOPortOutputArray[iPort]=(uint32)OutputCfgDigital<<16;
    }
    else
        *IOPortOutputArray[iPort]=(uint32)OutputCfgNoCfg<<16;
...
void MultiIOEB05Init(void){
    uint16 iPort;
    iPort=39;
    x05_out_set_relay (X_EXPBOARD_SLOT_C, X05_OUT_RELAY_01, X05_ON);
    //Input Initial
    while(iPort<NumOfInput){
        if(inputConfig[iPort][1]!=InputCfgNoCfg){
            x05_in_init(X_EXPBOARD_SLOT_C,inputConfig[iPort][0],X05_IN_TYPE_DIGITAL,0,(X05_IN_CIRCUIT_ACTIVE_HIGH|X05_IN_CIRCUIT_PULL_DOWN),0,NULL);
            *IOPortInputArray[iPort]=(uint32)InputCfgDigital<<16;
        }
        else
            *IOPortInputArray[iPort]=(uint32)InputCfgNoCfg<<16;
    ...
    while(iPort<NumOfOutput){
        if(outputConfig[iPort][1]!=OutputCfgNoCfg){
            x05_out_init(X_EXPBOARD_SLOT_C,outputConfig[iPort][0],X05_OUT_TYPE_DIGITAL,0,0,NULL);
            *IOPortOutputArray[iPort]=(uint32)OutputCfgDigital<<16;
        }
        else
            *IOPortOutputArray[iPort]=(uint32)OutputCfgNoCfg<<16;
    ...
    while(TRUE){
        if(flag==0){
            ...
            for(iPort=0;iPort<8;iPort++){
                ...
                tmpCFGType=outputConfig[iPort][1];
            ...
            for(iPort=15;iPort<27;iPort++){
                tmpCFGType=(uint16)((*IOPortInputArray[iPort]&0x00ff0000)>>16);
                if(tmpCFGType==InputCfgNoCfg) continue;
                x02_in_get_value(X_EXPBOARD_SLOT_A, inputConfig[iPort][0],&DIValue);
                *IOPortInputArray[iPort]&=0xffff0000;
            ...
            if(tmpCFGType==OutputCfgNoCfg) continue;
            DIValue=(sint32)(*IOPortOutputArray[iPort]&0x0000ffff);
            x02_out_set_value (X_EXPBOARD_SLOT_A, outputConfig[iPort][0], DIValue);
        }
        else if(flag==2){
            for(iPort=27;iPort<39;iPort++){
                tmpCFGType=(uint16)((*IOPortInputArray[iPort]&0x00ff0000)>>16);
                if(tmpCFGType==InputCfgNoCfg) continue;
                x02_in_get_value(X_EXPBOARD_SLOT_B, inputConfig[iPort][0] ,&DIValue);
                *IOPortInputArray[iPort]&=0xffff0000;
                *IOPortInputArray[iPort]+=(uint32)DIValue;
            }
            iPort=9;
            ...
            if(tmpCFGType==OutputCfgNoCfg) continue;
            DIValue=(sint32)(*IOPortOutputArray[iPort]&0x0000ffff);
            x02_out_set_value (X_EXPBOARD_SLOT_B, outputConfig[iPort][0], DIValue);
        }
        else if(flag==3){
            for(iPort=39;iPort<NumOfInput;iPort++){
                tmpCFGType=(uint16)((*IOPortInputArray[iPort]&0x00ff0000)>>16);
                if(tmpCFGType==InputCfgNoCfg) continue;
                x05_in_get_value(X_EXPBOARD_SLOT_C, inputConfig[iPort][0] ,&DIValue);
                *IOPortInputArray[iPort]&=0xffff0000;
                *IOPortInputArray[iPort]+=(uint32)DIValue;
            }
            for(iPort=10;iPort<NumOfOutput;iPort++){
                tmpCFGType=(uint16)((*IOPortOutputArray[iPort]&0x00ff0000)>>16);
                tmpCFGType=outputConfig[iPort][1];
                if(tmpCFGType==OutputCfgNoCfg) continue;
                DIValue=(sint32)(*IOPortOutputArray[iPort]&0x0000ffff);
                x05_out_set_value (X_EXPBOARD_SLOT_C, outputConfig[iPort][0], DIValue);
            }
        }
        flag++;
        if(flag==4) flag=0;
...
void InitMultiIO(void){
    uint8 i;
    uint8 StartSubindex;
    IOPortControlWord=BindODMem(IOPortStartIndex,SubindexControlWord,TypeU32,0); *IOPortControlWord=0x0000;
